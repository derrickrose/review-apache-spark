...
print(
    "-------------------------------------------- showing serie listing_url top 3--------------------"
)
...
print(
    "-------------------------------------------- showing serie to get values of it --------------------"
)
...
print(
    "-------------------------------------------- showing serie of the comparison to 't' --------------------"
)
...
print(
    "-------------------------------------------- showing serie with value different from 't' --------------------"
)
...
print(
    "-------------------------------------------- showing serie with multiple condition --------------------"
)
...
print(
    len(df[df["host_is_superhost"] == "f"])
)  # 232 not superhost , add with previous value which is 229 461 => what is the 4 rest????
print("-------------------------------- not superhost true nor superhost false")
print(len(df[~((df["host_is_superhost"] == "t") | (df["host_is_superhost"] == "f"))]))
print(
    df[
        ~((df["host_is_superhost"] == "t") | (df["host_is_superhost"] == "f"))
    ].to_string()
)  # values are NaN
...
print(
    reset_df.columns
)  # Index(['beds', 'price'], dtype='object') beds column is back again after the reset_index()
...
print("-------------------------------------------- sorting --------------------")
print(
    reset_df["price"].sort_values(ascending=False)
...
print(
    nan_df.fillna({"beds": "0", "price": "$0"}, inplace=True)
)  # None since directly the dataframe was modified
print(
    nan_df.to_string()
)  # actually contains $0 so proof that inplace update directly the DataFrame itself
