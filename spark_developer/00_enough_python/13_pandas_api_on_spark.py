...
spark_df = spark_session.read.csv(
    "listings.csv", header="true", inferSchema="true", multiLine="true", escape='"'
)
...
pypandas_df = ps.read_csv(
    "listings.csv", inferSchema="true", header=0, multiLine="true", escape='"'
)
...
)  ## pyspark.pandas modern, preferred and safer
...
print(
    "-------------------------------------------- spark dataframe property count --------------"
)
...
result = ps.sql(
    "SELECT DISTINCT(property_type) FROM {pypandas_df}", pypandas_df=pypandas_df
)
