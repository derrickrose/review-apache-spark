...
name_series2 = (
    df.Name
)  # not best practice since if the column name contains space, this approach wont work
print("----------------------------------------------------name series2")
print(name_series2)
...
## e.g.
print(
    "------------------------------------------------------------ more operations on numerical serie"
)
age_series = df["Age"]
print(age_series)
...
print(
    age_series * 3 - 2
)  # will result a series with the values  multiplied by 3 then menus 2
...
df1 = df[
    ["Name", "Age"]
]  # this is a view or a slice of dataframe df not a new completely separated dataframe
...
print(
    df1["Name"].str.upper()
)  ## print(df1[["Name"]].str.upper())  AttributeError 'DataFrame' object has no attribute 'str'
## to do so
print(
    df1[["Name"]].apply(lambda col: col.str.upper())
)  # apply will not change the actual dataframe, returns a transformed copy
